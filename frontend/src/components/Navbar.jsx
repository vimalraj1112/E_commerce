...
const Navbar = () => {
    ...
    return (
        <nav className="bg-neutral-900/95 backdrop-blur-xl sticky top-0 z-50 border-b border-white/10 shadow-lg shadow-neutral-900/10">
                ...
                <div className="flex justify-between h-20">
                    <div className="flex items-center">
                        <Link to="/" className="flex-shrink-0 flex items-center group transition-all">
                            <motion.span
                                whileHover={{ rotate: 8 }}
                                className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-tr from-neutral-700 to-neutral-500 shadow-lg shadow-neutral-900/40"
                            >
                                <ShoppingBag className="h-5 w-5 text-white" />
                            </motion.span>
                            <span className="ml-3 text-2xl font-black text-white tracking-tighter font-display drop-shadow-sm">
                                Mini<span className="text-neutral-300">Shop</span>
                            </span>
                        </Link>
                        ...
                        {user ? (
                            <>
                                        ...
                                        <AnimatePresence>
                                            {cartCount > 0 && (
                                                <motion.span
                                                    ...
                                                    className="absolute -top-0.5 -right-0.5 min-w-[1.1rem] h-4.5 px-1 bg-gradient-to-tr from-neutral-600 to-neutral-400 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-lg"
                                                >
                                                    {cartCount}
                                                </motion.span>
                                            )}
                                ...
                                <div className="flex items-center space-x-2.5 bg-white/15 backdrop-blur px-4 py-2 rounded-2xl border border-white/20 shadow-sm">
                                    <div className="bg-white p-1.5 rounded-lg text-neutral-700">
                                        <User className="h-4 w-4" />
                                    </div>
                                    <div className="leading-none">
                                        <span className="text-sm font-bold text-white">{user.name}</span>
                                        {isAdmin && (
                                            <p className="text-[10px] font-black text-neutral-300 uppercase tracking-wider mt-0.5">Admin</p>
                                        )}
                                    </div>
                                </div>
                                <button
                                    onClick={handleLogout}
                                    className="p-2 text-white/80 hover:text-neutral-200 hover:bg-white/10 rounded-xl transition-all"
                                    title="Logout"
                                >
                            ...
                            <div className="space-x-4 flex items-center">
                                ...
                                <motion.span whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} className="inline-block">
                                    <Link to="/register" className="flex items-center gap-1.5 bg-gradient-to-tr from-neutral-800 to-neutral-600 text-white px-6 py-2.5 rounded-xl font-bold hover:from-neutral-700 hover:to-neutral-500 transition-all shadow-lg shadow-neutral-900/30 active:scale-95">
